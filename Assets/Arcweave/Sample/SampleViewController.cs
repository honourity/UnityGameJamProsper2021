...
public class SampleViewController : MonoBehaviour
{
    ...
    private void Speak(Element element)
    {
        VoiceManager.Instance.Speak(element);
    }

    /*
     * Populate components from element.
     */
    private void PopulateComponents(Element element)
    {
        // Clean up old ones
        ...
        componentParent.SetActive(element.components.Length > 0);
        if (element.components.Length > 0) {
            for (int i = 0; i < element.components.Length; i++) {
                ...
                label.text = element.components[i].realName;

                // The mask
                Sprite sprite = element.components[i].cover;
                ...
                if (sprite != null) {
                    ...
                    icon.sprite = element.components[i].cover;
                    icon.SetNativeSize();
                ...
                AW.Component awComponent = element.components[i];
